...
def load_gcn_subgraph(dir):
    ...
    with open(dir) as f:
        ...
        for line in tqdm(data):
            ...
            qid = int(query_all[0])
            query_pos_text, query_neg_text, query_pos_neighbors, query_neg_neighbors = [qq.split('\t') for qq in
                                                                                        query_all[1:]]
...
class EdgeFormerEncoderN(nn.Module):
    def __init__(self, config):
        super(EdgeFormerEncoderN, self).__init__()
...
class EdgeFormersN(BertPreTrainedModel):
    def __init__(self, config):
        super(EdgeFormersN, self).__init__(config=config)
        self.config = config
        self.hidden_size = config.hidden_size
        self.embeddings = BertEmbeddings(config=config)
        self.encoder = EdgeFormerEncoderN(config=config)
...
class EdgeFormersForNeighborPredict(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.bert = EdgeFormersN(config)
        self.hidden_size = config.hidden_size
        self.init_weights()
